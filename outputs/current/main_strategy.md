...
- Patch SLA is the governance bridge between vulnerability discovery, product trust, patient-safety continuity, and audit readiness.

### Risk Factors And Mitigations

| Risk Factor | Mitigation |
| --- | --- |
| Regulatory content can become dense and slow if it sounds like paperwork instead of evidence. | Compress regulation into evidence-chain logic: architecture -> threat model -> control -> test -> finding decision -> patch/retest record. |
| Technical examples can drift toward tool catalogs instead of decision questions. | Use product-boundary comparisons instead of vulnerability lists. |
| Slides 6 and 7 can overload the audience if they become framework/control lists instead of evidence and risk-layer logic. | Keep national strategy as altitude and financial resilience as analogy; return immediately to medical AI clinical continuity. |
| National strategy or financial-sector references can become scope creep if they are presented as a second topic. | Make Slide 10 an intentional recovery beat after the dense middle. |
| The ending can be weakened if the speaker spends too long on testing details. | Use cut markers before speaking faster. |
| Any private implementation detail would reduce trust and public-safety readiness. | Keep all examples public-safe, source-anchored, and artifact-level. |
...
| --- | --- |
| Mode | myth -> consequence -> boundary -> evidence -> governance -> implementation |
| Why this mode | The audience likely begins with the false simplification that AI SaMD security is model security or compliance paperwork. This structure breaks that myth, shows real clinical and operating consequence, then gives a decision-grade model for product boundary, evidence, testing, findings, and repairability. |
...
| Problem framing | The security problem grows as AI moves from model artifact to viewer, platform, database, connected medical system, update chain, and support workflow. Taiwan's national cybersecurity context puts AI security, critical infrastructure, key industries, and supply-chain resilience in one frame; this talk uses medical AI SaMD as the concrete healthcare case. Each boundary crossing adds attack surface, evidence burden, and management decisions. |
| Tension | Regulators, hospitals, and users do not only ask whether the model is accurate. They ask whether the product can be monitored, patched, disclosed, tested, explained, repaired, and kept available enough for clinical workflow. |
| Climax | Slide 12: every finding needs ownership, severity, a decision, a repair path, retest, and archived evidence. This is where testing, regulation, product ownership, and trust converge. |
| Closing structure | Slide 13 gives the audience a measurable 30 / 60 / 90 evidence loop; this is an adoption scaffold, not a mandatory legal or SLA format. Slide 14 closes with trust-before-audit and three readiness questions. |
| Final takeaway | 先把安全做進產品，法規文件才會自然長出來。 |

### Development Arc

- Slide 3 breaks the model-only myth.
- Slide 4 turns AI infrastructure and Taiwan's AI/critical-infrastructure resilience context into clinical-continuity consequence.
- Slide 5 installs product boundary as the master decision variable.
- Slide 6 translates official obligations into traceable evidence artifacts.
- Slide 7 separates model behavior, software stack, and clinical workflow while using NIST as shared language.
- Slides 8-9 apply the boundary model to concrete product scales, with finance and other critical sectors used only as continuity analogies.
- Slide 10 turns the technical argument into patient-safety significance.
- Slide 11 makes testing a portfolio of decision questions.
- Slide 12 converts findings into Patch SLA governance and borrows resilience language from mature regulated sectors without changing the healthcare focus.
- Slides 13-14 give an executable first 90-day evidence loop and final readiness checklist.
...
| FDA Contec and Epsimed patient monitor safety communication | https://www.fda.gov/medical-devices/safety-communications/cybersecurity-vulnerabilities-certain-patient-monitors-contec-and-epsimed-fda-safety-communication |
