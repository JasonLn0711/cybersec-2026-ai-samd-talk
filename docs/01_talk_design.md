...
## National And Cross-Sector Framing

This talk may reference Taiwan's `國家資通安全戰略2025`, the `國家資通安全發展方案（114年至117年）`, critical-infrastructure resilience, and financial-sector cybersecurity resilience. These references should raise the strategic altitude without changing the talk's center of gravity.

Use this framing:

- National strategy: AI security, critical infrastructure, key industries, and supply-chain resilience are converging.
- Healthcare: medical AI SaMD is the primary case because cyber risk can become clinical-continuity and patient-safety risk.
- Finance: financial cybersecurity resilience is a useful governance analogy because it already speaks in continuity, accountability, backup, drills, supply-chain oversight, and measurable recovery.
- Boundary: do not turn the talk into a national-strategy survey or a financial-security lecture.

Stage line:

`國家戰略告訴我們，AI、關鍵基礎設施、供應鏈韌性正在合流；醫療 AI SaMD 是這個合流裡最需要具體落地的一種產品場景。`
...
| Cross-functional managers | Shared ownership language without heroics. | Governance language, decision records, small-team path, and financial-sector resilience as a compact analogy. |
...
Install Node dependencies once, then build the editable deck:
...
| 4 | AI infrastructure becomes clinical-continuity risk | AI stack plus continuity break. | `台灣的國家資安脈絡也在提醒我們：AI、關鍵基礎設施與供應鏈韌性正在合流。` | Keep only AI infrastructure, national-context bridge, workflow, disruption. |
...
| 9 | Scale 3-4: Platform to Connected System | Platform vs clinical environment map. | `金融業談交易不中斷，醫療這裡談的是 clinical continuity。` | Keep identity/API/database, hospital network/update server, and one cross-sector analogy. |
...
台灣自己的國家資安脈絡也在往同一個方向走。`國家資通安全戰略2025` 把 AI 應用與安全、關鍵基礎設施、關鍵產業與供應鏈放在同一個戰略框架裡；第七期國家資通安全發展方案也把全社會資安防禦、關鍵基礎設施韌性、資安產業和 AI 新興資安科技放在一起看。

所以這裡不是要多講一段國家戰略，而是提醒我們：醫療 AI 正好站在 AI、關鍵基礎設施、供應鏈韌性的交界。
...
這裡可以借金融業的一句話來對照：金融業談交易不中斷，通訊業談連線不中斷，能源談供應不中斷；醫療這裡，我們談的是 clinical continuity，照護流程不能被資安事件輕易打斷。
...
金融資安韌性的語言其實很值得借鏡：治理、問責、演練、備援、供應鏈、可恢復。醫療 AI 團隊不必一開始就變成大型金融機構，但至少要建立同一個方向的最小治理迴圈。
...
`30 / 60 / 90` 不是法規要求，也不是 Patch SLA 一定要長這樣。它是演講裡的 adoption scaffold：讓主管、PM、RD、QA 和資安人員不用先討論大型制度，而能先對齊第一個 `90` 天要產生的證據。

- `30` days: make the product visible enough to reason about.
- `60` days: make risk flow into a repeatable workflow.
- `90` days: validate the workflow externally and preserve repair evidence.

If the audience is a mature enterprise, rename it `first 90-day evidence loop`. If the audience is an early startup, keep `30 / 60 / 90` because it lowers the psychological entry cost.
