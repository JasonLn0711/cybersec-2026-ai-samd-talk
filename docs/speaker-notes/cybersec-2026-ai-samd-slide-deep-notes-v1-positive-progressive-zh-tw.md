...
「如果一個 segmentation model 在 validation set 上表現很好，但醫院使用時影像序列錯了、DICOM metadata 有缺漏、viewer 顯示 context 錯了、暫存資料留存過久，臨床信任仍然會受到影響。」
...
「第二種是 governance language：owner、severity、acceptance criteria、residual risk、release decision。NIST CSF 2.0 和 AI RMF 的價值，是提供跨角色共同語言，讓各角色能討論同一個風險。」
...
「這句話要被翻成醫療現場語言。Availability 是醫師需要結果時，系統是否可用。Integrity 是醫師是否能信任 AI output。Confidentiality 是病人資料與病人信任是否被保護。Repairability 是出事後產品是否能安全恢復。」
...
「Change Healthcare、Synnovis、馬偕、Contec 這些案例共同指向同一件事：醫療系統 availability 受損、資料 integrity 受損、漏洞修補受阻時，臨床流程會受到壓力。」
...
「SBOM / SCA 回答：我們用了什麼元件，是否存在已知漏洞？」

「White-box 回答：release 前有哪些便宜、可修、可追溯的問題？」

「Fuzz / abuse case 回答：異常輸入是否會打壞流程？」

「Black-box 回答：外部看得到什麼？」

「Pen test 回答：漏洞是否能被串成真實 attack path？」
...
「FDA 524B 落到公司內部，會變成一連串問題：漏洞進來誰收？誰判斷是否適用？誰定嚴重度？誰決定 fix now、compensating control、defer、scope exclusion？誰通知客戶？誰重測？證據放哪？」
...
「一般觀眾可以用房子理解：第一個月先畫平面圖，第二個月開始裝門鎖和監視器，第三個月找外部人員測試門鎖是否有效。」
...
「一個 8 人 AI 醫療團隊可以這樣跑：30 天發現 DICOM cache 缺少 retention policy；60 天用 STRIDE 找到 model tampering 和 unauthorized PACS access；90 天用 black-box pen test 找到 login interface 的限制偏弱，先用 IP restriction / firewall compensating control 降風險，再安排正式修補與重測。」
